...
public class ResetPasswordControl extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        Users account = dAOUsers.getAccountByEmail(email);
        
        String newPass = request.getParameter("password");
        String confirmNewPass = request.getParameter("confPassword");
        
        if(!newPass.equals(confirmNewPass)){
            String errDuplicatePass = "You must be enter duplicate pass";
            request.setAttribute("errDuplicatePass", errDuplicatePass);
            request.getRequestDispatcher("newPassword.jsp").forward(request, response);
        }else{
             DAOLoginHistory loginHistoryDAO =  new DAOLoginHistory();
       
        Users user = (Users) session.getAttribute("acc");
        loginHistoryDAO.insertRecentActivity(user.getId(),18); 
            dAOUsers.UpdatePassAccount(account, newPass);
