...
public class DeletePost extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       int postid=Integer.parseInt(postid_str); 
        ...
        request.setAttribute("tb", "Delete success !");
       }
        HttpSession session = request.getSession();
        Users user = (Users) session.getAttribute("acc"); 
        if(user.getRoleId()==1) request.getRequestDispatcher("AdManagerForum").forward(request, response); 
        else request.getRequestDispatcher("getAllPost").forward(request, response); 
    } 
